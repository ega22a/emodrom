...
Веб-сервис для психологической party-игры про эмоции. Три экрана:

- **Пульт ведущего** — ведущий создаёт лобби и управляет игрой прямо со
  своего телефона: настраивает сессию (банк вопросов, число раундов,
  допрос, набор эмоций), запускает раунды, показывает результаты,
  проводит допрос.
- **Витрина** — публичный экран без единой кнопки управления, для проектора.
  QR-код для входа, раскрытие результатов раунда с анимацией.
- **Экран игрока** — телефон каждого участника: вход по QR, выбор
  аватара, ответы в раунде.

Каждый раунд один игрок — чтец — читает вслух карточку-ситуацию и
приватно выбирает эмоцию, которую испытал бы на месте героя. Остальные
игроки честно выбирают эмоцию, которую испытали бы сами — это не
угадайка. Совпавшие с чтецом получают «искры» (внутриигровую валюту) в
размере награды карточки; если не совпал никто, искры забирает чтец.
Искры тратятся по 3 штуки на случайную новую эмоцию. Экран ведущего и
витрина обновляются в реальном времени через WebSocket (Laravel Reverb).
...
Банки вопросов и эмоции — это данные, не код: живут в сидерах
`database/seeders/QuestionBankSeeder.php` (сейчас там один банк-заглушка
«Разные ситуации») и `database/seeders/EmotionSeeder.php` (базовые +
классические + расширенные эмоции). Чтобы добавить свой банк вопросов —
допишите новый массив в `QuestionBankSeeder` (текст ситуации + награда в
искрах для каждой) и запустите `php artisan db:seed --class=QuestionBankSeeder`.
